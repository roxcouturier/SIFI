...
PFS <- read.csv("./Figures/Figure 1/Données PFS.csv", sep=";")
...
plot_PFS <- ggsurvplot(survfit(Surv(time_month,event_month)~Trt,data=PFS), data=PFS, pval = T, risk.table = T, ncensor.plot = TRUE  ,palette  = c("#e4342c","#1c246c")) + xlab("Time in months") + ylab("PFS survival probability")  


####OS 


OS <- read.csv("./Figures/Figure 1//Données OS .csv", sep=";")
...
plot_OS <- ggsurvplot(survfit(Surv(time_month,event_month)~Trt,data=OS), data=OS, pval = T, risk.table = T, ncensor.plot = TRUE  ,palette  = c("#e4342c","#1c246c")) + xlab("Time in months") + ylab("OS survival probability")  
