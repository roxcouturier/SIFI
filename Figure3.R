...
files_names <- list.files(path = here("/Users/roxanecouturier/Desktop/SIFI/Figures/Figure 3/SIFI_n_40_640"))
...
best_flip_graph <- best_flip %>%
  ...
  labs(fill = "", title = "") + 
  scale_x_continuous(breaks = seq(min(best_flip$sifi), max(best_flip$sifi), by = 1),
                  labels = seq(min(best_flip$sifi), max(best_flip$sifi), by = 1)) +
  theme(
    legend.text = element_text(size = 25),
    legend.title = element_text(size = 25),
    text = element_text(size = 25),
    plot.title = element_text(size = 12),
    axis.text.x = element_text(size = 20),   # Increase x-axis label text size
    axis.text.y = element_text(size = 20), 
    axis.title.x = element_text(size = 25, hjust = 0.5),           # Taille du titre de l'axe x
